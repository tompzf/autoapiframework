...
cd component_designer/AutomotiveComponentDesigner
...
| `src/MainFrame.*` | Window, buttons and the four collection views |
| `src/YamlNode.*` | Order preserving YAML document tree |
| `src/YamlParser.*` | Block style YAML subset parser |
| `src/YamlWriter.*` | Emitter reproducing the `.acs` layout |
| `src/MetaModel.*` | `autoapiframework_meta_model.yaml` (enums, interface types) |
| `src/FunctionSpecification.*` | Function specification access and load/save |
