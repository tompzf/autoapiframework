...
MainFrame::MainFrame()
    : wxFrame(nullptr, wxID_ANY, "AutomotiveComponentDesigner", wxDefaultPosition, wxSize(1200, 750)) {
    BuildUi();
    AutoLoadMetaModel();
...
void MainFrame::OnSaveSpecificationAs(wxCommandEvent&) {
    if (!m_specification.IsLoaded()) {
        wxMessageBox("Read a function specification first.", "AutomotiveComponentDesigner",
    ...
    if (!m_specification.Save(ToStd(dialog.GetPath()), error)) {
        wxMessageBox("Could not write file:\n" + ToWx(error), "AutomotiveComponentDesigner",
...
void MainFrame::OnAbout(wxCommandEvent&) {
    wxMessageBox("AutomotiveComponentDesigner\n\n"
                 ...
                 "About AutomotiveComponentDesigner", wxOK | wxICON_INFORMATION, this);
}
...
void MainFrame::LoadSpecification(const wxString& path) {
    ...
    if (!specification.Load(ToStd(path), error)) {
        wxMessageBox("Could not read function specification:\n" + ToWx(error),
                     "AutomotiveComponentDesigner", wxOK | wxICON_ERROR, this);
        return;
    }
